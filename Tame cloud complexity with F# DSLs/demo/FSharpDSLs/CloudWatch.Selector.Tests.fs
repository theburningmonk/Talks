module CloudWatchSelectorTests
...
[<Test>]
let ``tokenize should split tokens by space`` () =
    tokenize "Mary had a little lamb"
    |> should equal [ "Mary"; "had"; "a"; "little"; "lamb" ]

[<Test>]
let ``tokenize should handle leading whitespaces`` () =
    tokenize "  Mary had a little lamb"
    |> should equal [ "Mary"; "had"; "a"; "little"; "lamb" ]

[<Test>]
let ``tokenize should handle trailing whitespaces`` () =
    tokenize "Mary had a little lamb   "
    |> should equal [ "Mary"; "had"; "a"; "little"; "lamb" ]

[<Test>]
let ``tokenize should handle duplicated whitespaces`` () =
    tokenize "Mary   had   a   little   lamb"
    |> should equal [ "Mary"; "had"; "a"; "little"; "lamb" ]

[<Test>]
let ``tokenize should handle spaces between single quotes`` () =
    tokenize "the time was '1997-09-01 12:00:04' "
    |> should equal [ "the"; "time"; "was"; "'1997-09-01 12:00:04'" ]

[<Test>]
let ``namespaceIs should create a predicate for case-insensitive equality comparison`` () =
...
[<Test>]
let ``namespaceLike should create a predicate for case-insensitive regex match`` () =
...
[<Test>]
let ``nameIs should create a predicate for case-insensitive equality comparison`` () =
...
[<Test>]
let ``nameLike should create a predicate for case-insensitive regex match`` () =
...
[<Test>]
let ``unitIs ahould create a predicate for case-insensitive equality comparison`` () =
...
[<Test>]
let ``average should create a predicate for numeric comparison`` () = testStatsFilter "average" Average

[<Test>]
let ``min should create a predicate for numeric comparison`` () = testStatsFilter "min" Min

[<Test>]
let ``max should create a predicate for numeric comparison`` () = testStatsFilter "max" Max

[<Test>]
let ``sum should create a predicate for numeric comparison`` () = testStatsFilter "sum" Sum

[<Test>]
let ``sampleCount should create a predicate for numeric comparison`` () = testStatsFilter "sampleCount" SampleCount
    
[<Test>]
let ``dimensionContains should create a dimension filter`` () =
...
[<Test>]
let ``duringLast should create a timeframe for last x minutes`` () =
...
[<Test>]
let ``since should create a timeframe with the specified start time`` () =
...
[<Test>]
let ``between should create a timeframe with the specified start and end time`` () =
...
[<Test>]
let ``multiple filters should be combined with 'and'`` () =
...
[<Test>]
let ``intervalOf should create a period with the specified unit of time`` () =
